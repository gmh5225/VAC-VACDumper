...
void OnStart()
{
	...
	uint8_t* signature = Utility::FindMemorySequence("steamservice.dll", "55 8B EC 83 EC ? 53 56 8B 75 ? 8B D9 83 7E");
	...
	MH_Initialize();
	MH_CreateHook(signature, &Hooks::LoadVACModule, reinterpret_cast<LPVOID*>(&Hooks::oLoadVACModule));
	MH_EnableHook(MH_ALL_HOOKS);

	status += 1;
	std::cout << "[" << status << "/4] " << "Hook placed on 0x" << reinterpret_cast<uintptr_t*>(signature) << "\n" << std::endl;
}
